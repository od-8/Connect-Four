...
class Board
  ...
  def diagonal_win?
    @board.each_with_index do |list, list_index|
      list.my_each_with_index do |_node, node_index|
        return false if list_index == 5

        if node_index > 3
          return true if @board[list_index].at_index(node_index).value == "X" &&
                         @board[list_index + 1].at_index(node_index + 1).value == "X" &&
                         @board[list_index + 2].at_index(node_index + 2).value == "X" &&
                         @board[list_index + 3].at_index(node_index + 3).value == "X"

          return true if @board[list_index].at_index(node_index).value == "O" &&
                         @board[list_index + 1].at_index(node_index - 1).value == "O" &&
                         @board[list_index + 2].at_index(node_index - 2).value == "O" &&
                         @board[list_index + 3].at_index(node_index - 3).value == "O"
        end
